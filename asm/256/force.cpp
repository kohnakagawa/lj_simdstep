...
//----------------------------------------------------------------------
void
force_sorted_intrin_mat_transp(void) {
  const v4df vzero = _mm256_set_pd(0, 0, 0, 0);
  const v4df vcl2 = _mm256_set_pd(CL2, CL2, CL2, CL2);
  const v4df vc24 = _mm256_set_pd(24 * dt, 24 * dt, 24 * dt, 24 * dt);
  const v4df vc48 = _mm256_set_pd(48 * dt, 48 * dt, 48 * dt, 48 * dt);
...
force_sorted_z_swp(void) {
  ...
  const v4df vc48 = _mm256_set_pd(48 * dt, 48 * dt, 48 * dt, 48 * dt);
  for (int i = 0; i < pn; i++) {
    const v4df vqi = _mm256_load_pd(&z[i][X]);
    v4df vpf = _mm256_set_pd(0.0, 0.0, 0.0, 0.0);
    const int kp = pointer[i];
    int ja_1 = sorted_list[kp];
    int ja_2 = sorted_list[kp + 1];
    int ja_3 = sorted_list[kp + 2];
    int ja_4 = sorted_list[kp + 3];
    v4df vqj_1 = _mm256_load_pd(&z[ja_1][X]);
    v4df vdqa_1 = vqj_1 - vqi;
    v4df vqj_2 = _mm256_load_pd(&z[ja_2][X]);
    v4df vdqa_2 = vqj_2 - vqi;
    v4df vqj_3 = _mm256_load_pd(&z[ja_3][X]);
    v4df vdqa_3 = vqj_3 - vqi;
    v4df vqj_4 = _mm256_load_pd(&z[ja_4][X]);
    v4df vdqa_4 = vqj_4 - vqi;
    ...
    for (int k = 0; k < (np / 4) * 4; k += 4) {
      ...
      vqj_1 = _mm256_load_pd(&z[ja_1][X]);
      vdqa_1 = vqj_1 - vqi;
      vpf += vdf_1 * vdqb_1;

      v4df vpjb_1 = _mm256_load_pd(&z[jb_1][PX]);
      vpjb_1 -= vdf_1 * vdqb_1;
      _mm256_store_pd(&z[jb_1][PX], vpjb_1);

      vqj_2 = _mm256_load_pd(&z[ja_2][X]);
      vdqa_2 = vqj_2 - vqi;
      vpf += vdf_2 * vdqb_2;

      v4df vpjb_2 = _mm256_load_pd(&z[jb_2][PX]);
      vpjb_2 -= vdf_2 * vdqb_2;
      _mm256_store_pd(&z[jb_2][PX], vpjb_2);

      vqj_3 = _mm256_load_pd(&z[ja_3][X]);
      vdqa_3 = vqj_3 - vqi;
      vpf += vdf_3 * vdqb_3;

      v4df vpjb_3 = _mm256_load_pd(&z[jb_3][PX]);
      vpjb_3 -= vdf_3 * vdqb_3;
      _mm256_store_pd(&z[jb_3][PX], vpjb_3);

      vqj_4 = _mm256_load_pd(&z[ja_4][X]);
      vdqa_4 = vqj_4 - vqi;
      vpf += vdf_4 * vdqb_4;

      v4df vpjb_4 = _mm256_load_pd(&z[jb_4][PX]);
      vpjb_4 -= vdf_4 * vdqb_4;
      _mm256_store_pd(&z[jb_4][PX], vpjb_4);
    ...
    v4df vpi = _mm256_load_pd(&z[i][PX]);
    vpf += vdf_1 * vdqb_1;
    vpf += vdf_2 * vdqb_2;
    vpf += vdf_3 * vdqb_3;
    vpf += vdf_4 * vdqb_4;
    vpi += vpf;
    _mm256_store_pd(&z[i][PX], vpi);
...
main(void) {
  ...
  print_result();
#elif S_INTRIN_M_TRANS
  measure(&force_sorted_intrin_mat_transp, "sorted_intrin_mat_transp");
  print_result();
#elif S_SWP_INTRIN
